      ...
      <div class="expand-report-card__content">
        ...
        >
          <div class="expand-report-card__meta-row">
            <h3 class="expand-report-card__title">{{ categoryLabels[report.category] }}</h3>
            <span class="expand-report-card__time">{{ formatRelativeTime(report.created_at) }}</span>
          </div>
          ...
          <p v-if="report.description" class="expand-report-card__description">
            {{ report.description }}
          </p>
...
import ReportSourceBadge from '@/components/dashboard/ReportSourceBadge.vue';
import type { ReportSummary } from '@/types/report';
import { categoryMarkerStyles } from '@/utils/categoryMapMarkers';
...
const categoryColor = computed(() => categoryMarkerStyles[props.report.category].fill);
const gridPatternId = computed(() => `expand-report-grid-${props.report.id}`);

const coordinatesLabel = computed(() => {
  if (!hasValidCoordinates(props.report)) {
    return 'Coordinates unavailable';
  }

  const { latitude, longitude } = props.report;
...
.expand-report-card--expanded .expand-report-card__pin {
  top: 50%;
}
...
.expand-report-card--expanded .expand-report-card__map-icon {
  opacity: 0;
}
...
.expand-report-card__bottom--expanded :deep(.app-badge--source-citizen) {
  color: var(--text-secondary);
  background: color-mix(in srgb, var(--text-primary) 7%, transparent);
  border-color: var(--status-new-border);
}

.expand-report-card__bottom--expanded :deep(.app-badge--source-ai-audit) {
  border-color: var(--status-new-border);
}
